...
/**
 * Calcule le prix d'un slot selon les tranches tarifaires dynamiques.
 *
 * Règles :
 * 1. On cherche une tranche active dont time_from <= slotStart < time_to
 *    ET dont le day_of_week du créneau est dans days_of_week.
 * 2. Si plusieurs tranches matchent (ne devrait pas arriver si bien configuré),
 *    on prend la plus spécifique = celle avec le moins de jours couverts.
 * 3. Si aucune tranche → fallback sur courtBasePrice.
 *
 * slotStart : Date object de l'heure de début du créneau
 * priceSlots : tableau des tranches (depuis la DB)
 * courtBasePrice : price_per_slot du terrain (fallback)
 *
 * Retourne { price, label, isDynamic }
 * isDynamic = true si un tarif dynamique a été appliqué
 */
export function calcSlotPrice(slotStart, priceSlots, courtBasePrice) {
  if (!priceSlots || priceSlots.length === 0) {
    return { price: courtBasePrice, label: null, isDynamic: false }
  }

  const jsDay = slotStart.getDay() // 0=Dim, 1=Lun...6=Sam
  const dayOfWeek = jsDay === 0 ? 6 : jsDay - 1 // 0=Lun..6=Dim
  const slotTime = slotStart.toTimeString().substring(0, 5) // "HH:MM"

  const matching = (priceSlots || [])
    .filter(ps => {
      if (!ps.is_active) return false
      if (!ps.days_of_week.includes(dayOfWeek)) return false
      const from = ps.time_from.substring(0, 5)
      const to = ps.time_to.substring(0, 5)
  ...
  // Plus spécifique = moins de jours couverts
  const best = matching.reduce((prev, curr) =>
    curr.days_of_week.length < prev.days_of_week.length ? curr : prev
  )
...
export function evaluateAccessRules(rules, userContext, date, time) {
  ...
  // Admin : bypass total — aucune règle d'accès horaire ne s'applique
