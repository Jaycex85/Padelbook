...
export default function Sidebar({ profile }) {
  ...
  return (
    <aside style={{ width: 'var(--sidebar-w)', background: 'var(--surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, height: '100vh', flexShrink: 0 }}
      ...
      {/* Logo */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '20px 16px', borderBottom: '1px solid var(--border)', fontFamily: "'Syne', sans-serif", fontSize: '18px', fontWeight: 700, color: 'var(--green)' }}>
        🎾 PadelBook
      </div>
      ...
      <nav style={{ flex: 1, padding: '12px 8px', overflowY: 'auto' }}>
        ...
        {playerLinks.map(link => (
          <Link key={link.href} href={link.href}
            style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 10px', borderRadius: '8px', textDecoration: 'none', color: pathname === link.href ? 'var(--green)' : 'var(--muted)', background: pathname === link.href ? 'var(--green-dim)' : 'none', fontSize: '14px', marginBottom: '2px', transition: 'all .15s' }}>
