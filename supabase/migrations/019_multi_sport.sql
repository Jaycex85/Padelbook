...
-- Idempotent : safe à ré-exécuter même si sport_type / courts.sport
-- ont déjà été créés manuellement.
...
-- héritent du sport via le court_id — pas de duplication de colonne ailleurs.
--
...
-- à la création de tout nouveau terrain (no-op si déjà retiré).
alter table courts
...
create index if not exists club_events_sport_idx on club_events(sport);
create index if not exists club_event_series_sport_idx on club_event_series(sport);
create index if not exists club_posts_sport_idx on club_posts(sport);
