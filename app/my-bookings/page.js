...
const STATUS_STYLES_BASE = {
  confirmed: { bg: 'var(--brand-dim)', color: 'var(--brand-light)' },
  pending: { bg: 'rgba(252,211,77,0.1)', color: 'var(--amber)' },
  cancelled: { bg: 'rgba(248,113,113,0.1)', color: 'var(--red)' },
  completed: { bg: 'rgba(139,148,158,0.1)', color: 'var(--muted)' },
  expired: { bg: 'rgba(139,148,158,0.1)', color: 'var(--muted)' },
}
...
function MyBookingsList() {
  ...
  const STATUS_STYLES = {
    confirmed: { ...STATUS_STYLES_BASE.confirmed, label: t('myBookings.statusConfirmed') },
    pending: { ...STATUS_STYLES_BASE.pending, label: t('myBookings.statusPending') },
    cancelled: { ...STATUS_STYLES_BASE.cancelled, label: t('myBookings.statusCancelled') },
    completed: { ...STATUS_STYLES_BASE.completed, label: t('myBookings.statusCompleted') },
    expired: { ...STATUS_STYLES_BASE.expired, label: t('myBookings.statusExpired') },
  }
  ...
  function openConfirm(booking, mode) {
    if (!canCancelBooking(booking)) {
      alert(t('myBookings.cancelDeadlineError'))
      return
    }
  ...
  async function inviteMember(member) {
    ...
    if (spotsLeft <= 0) { alert(t('myBookings.matchFull')); return }
  ...
  async function settleViaWallet(booking) {
    ...
    if (profErr) { console.error('settleViaWallet: lecture profil échouée', profErr); alert(t('payment.genericError', { message: profErr.message })); setSettling(null); return }
    const available = ownerProfile?.wallet_balance || 0

    if (available < openBalance) {
      alert(t('payment.insufficientWallet', { available: available.toFixed(2), required: openBalance.toFixed(2) }))
      setSettling(null)
      return
    }
    ...
    for (const p of unpaidAssigned) {
      ...
      if (playerErr || !upd || upd.length === 0) {
        ...
        for (const id of createdPlaceholderIds) await supabase.from('booking_players').delete().eq('id', id)
        alert(t('payment.blockedByRlsSupport', { id: p.id }))
        setSettling(null)
        return
      }
  ...
  const memberName = p => p.profile ? (p.profile.first_name || p.profile.email || t('nav.player')) : (p.guest_name ? p.guest_name + ' (' + t('myBookings.guest').toLowerCase() + ')' : t('nav.player'))

  if (loading) return <div style={{ textAlign: 'center', padding: '48px', color: 'var(--muted)' }}>{t('common.loading')}</div>

  return (
    <div>
      <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: '22px', fontWeight: 700, marginBottom: '24px' }}>{t('myBookings.title')}</h1>
      ...
      {bookings.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '64px', color: 'var(--muted)' }}>
          ...
          <p style={{ marginBottom: '16px' }}>{t('myBookings.noBookings')}</p>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px', flexWrap: 'wrap' }}>
                      ...
                      {b.is_public && <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '99px', background: 'rgba(252,211,77,0.1)', color: 'var(--amber)' }}>{t('myBookings.public')}</span>}
                      {!isOwner && <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '99px', background: 'var(--surface2)', color: 'var(--muted)' }}>{t('myBookings.guest')}</span>}
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontSize: '12px', color: 'var(--muted)' }}>{paidCount}/{b.players?.length || 1} {paidCount !== 1 ? t('myBookings.playersPaidPlural') : t('myBookings.playersPaid')}</span>
                          ...
                          <span key={p.id} style={{ fontSize: '11px', padding: '3px 9px', borderRadius: '99px', background: p.payment_status === 'paid' ? 'var(--brand-dim)' : 'var(--surface2)', color: p.payment_status === 'paid' ? 'var(--brand-light)' : 'var(--muted)' }}>
                            {memberName(p)} {p.payment_status === 'paid'
                              ? (b.payment_mode === 'full' && !p.is_owner
                                  ? (() => { const o = (b.players || []).find(pl => pl.is_owner); return '(' + t('myBookings.paidByOwner', { name: (o?.profile?.first_name || t('myBookings.theOwner')) }) + ')' })()
                                  : '✓')
                              : '⏳'}
                          </span>
                        ))}
                      ...
                      return (
                        <div style={{ marginTop: '10px', background: 'var(--brand-dim)', border: '1px solid var(--brand)', borderRadius: '8px', padding: '8px 12px', fontSize: '12px', color: 'var(--brand-light)' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap' }}>
                            <span>{t('myBookings.yourShare')} : <strong>{myRow.effective_price.toFixed(2)} €</strong></span>
                            <button onClick={() => payMyShare(b, myRow)} disabled={payingShare === myRow.id}
                              ...
                              {payingShare === myRow.id ? '...' : t('myBookings.payMyShare')}
                            </button>
                          </div>
                        </div>
                      )
                    ...
                    {openBalance > 0 && isOwner && (
                      <div style={{ marginTop: '10px', background: 'rgba(252,211,77,0.06)', border: '1px solid rgba(252,211,77,0.2)', borderRadius: '8px', padding: '8px 12px', fontSize: '12px', color: 'var(--amber)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap' }}>
                          <span>{t('myBookings.uncoveredBalance')} : <strong>{openBalance.toFixed(2)} €</strong></span>
                          <button onClick={() => setPendingSettle(b)} disabled={settling === b.id}
                            ...
                            {settling === b.id ? '...' : t('myBookings.settleNow')}
                          </button>
                        </div>
                        <p style={{ fontSize: '10px', marginTop: '4px', opacity: 0.85 }}>
                          {t('myBookings.autoDebitWarning')}
                        </p>
                      </div>
                    )}

                    {deadlinePassed && canCancel && (
                      <p style={{ fontSize: '11px', color: 'var(--amber)', marginTop: '6px' }}>{t('myBookings.cancellationDeadlinePassed')}</p>
                    )}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', alignItems: 'flex-end' }}>
                    ...
                    {canCancel && isOwner && (
                      <button onClick={() => openConfirm(b, 'full')} disabled={cancelling === b.id || deadlinePassed}
                        ...
                        {cancelling === b.id ? t('myBookings.processing') : t('myBookings.cancelMatch')}
                      </button>
                    )}
                    {canCancel && !isOwner && (
                      <button onClick={() => openConfirm(b, 'leave')} disabled={cancelling === b.id || deadlinePassed}
                        style={{ background: 'none', border: '1px solid var(--border)', color: deadlinePassed ? 'var(--muted)' : 'var(--red)', borderRadius: '8px', padding: '6px 12px', fontSize: '12px', cursor: deadlinePassed ? 'not-allowed' : 'pointer', opacity: deadlinePassed ? 0.5 : 1 }}>
                        {cancelling === b.id ? t('myBookings.processing') : t('myBookings.leaveMatch')}
                      </button>
                    )}
                    {isOwner && ['pending', 'confirmed'].includes(b.status) && (
                      <button onClick={() => togglePublic(b)}
                        ...
                        {b.is_public ? t('myBookings.makePrivate') : t('myBookings.makePublic')}
                      </button>
                    )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '16px' }}
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: 'min(420px, calc(100vw - 32px))' }}>
            <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: '18px', fontWeight: 700, marginBottom: '12px' }}>
              {confirmTarget.mode === 'full' ? t('myBookings.cancelTitle') : t('myBookings.leaveTitle')}
            </h2>
            <p style={{ fontSize: '14px', color: 'var(--muted)', marginBottom: '16px' }}>
              {confirmTarget.mode === 'full' ? t('myBookings.cancelBody') : t('myBookings.leaveBody')}
            </p>
            {(() => {
              ...
              return refund > 0 ? (
                <div style={{ background: 'var(--brand-dim)', border: '1px solid var(--brand)', borderRadius: '10px', padding: '12px 14px', marginBottom: '20px', fontSize: '14px', color: 'var(--brand-light)' }}>
                  💳 {t('myBookings.totalRefund')} : <strong>{refund.toFixed(2)} €</strong>
                </div>
              ) : null
            })()}
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                {cancelling === confirmTarget.booking.id ? t('myBookings.processing') : (confirmTarget.mode === 'full' ? t('myBookings.confirmCancellation') : t('myBookings.confirmLeave'))}
              </button>
            </div>
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '16px' }}
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: 'min(420px, calc(100vw - 32px))', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: '18px', fontWeight: 700 }}>{t('myBookings.inviteTitle')}</h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                {inviteTarget?.payment_mode === 'full' && (
                  <span style={{ fontSize: '11px', background: 'var(--brand-dim)', color: 'var(--brand-light)', border: '1px solid var(--brand)', borderRadius: '99px', padding: '3px 8px' }}>
                    {(() => { const o = (inviteTarget.players || []).find(p => p.is_owner); return t('myBookings.courtPaidBy', { name: (o?.profile?.first_name || t('myBookings.theOwner')) + (o?.profile?.last_name ? ' ' + o.profile.last_name : '') }) })()}
                  </span>
                )}
            ...
            {inviteTab === 'member' ? (
              <>
                ...
                <div style={{ overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {searching && <p style={{ fontSize: '13px', color: 'var(--muted)', textAlign: 'center' }}>{t('myBookings.searching')}</p>}
                  {!searching && search.length >= 2 && searchResults.length === 0 && (
                    <p style={{ fontSize: '13px', color: 'var(--muted)', textAlign: 'center', padding: '12px 0' }}>{t('myBookings.noMemberFound')}</p>
                  )}
                  {searchResults.map(m => (
                ...
                <p style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '14px', textAlign: 'center' }}>
                  {inviteTarget?.payment_mode === 'full'
                    ? (() => { const o = (inviteTarget.players || []).find(p => p.is_owner); const name = (o?.profile?.first_name || t('myBookings.theOwner')) + (o?.profile?.last_name ? ' ' + o.profile.last_name : ''); return t('myBookings.fullModeNote', { name }) })()
                    : t('myBookings.splitModeNote')}
                </p>
              </>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', overflowY: 'auto' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: 500, color: 'var(--muted)', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.3px' }}>{t('myBookings.guestNameLabel')}</label>
                  <input type="text" value={guestName} onChange={e => setGuestName(e.target.value)} placeholder={t('myBookings.guestNamePlaceholder')}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: 500, color: 'var(--muted)', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.3px' }}>{t('myBookings.guestEmailLabel')}</label>
                ...
                {inviteTarget?.payment_mode !== 'full' && (
                  <div style={{ background: 'rgba(252,211,77,0.06)', border: '1px solid rgba(252,211,77,0.2)', borderRadius: '8px', padding: '10px 12px', fontSize: '12px', color: 'var(--amber)' }}>
                    {t('myBookings.guestPaidByYou', { you: t('myBookings.you'), price: inviteTarget?.price_per_player?.toFixed(2) })}
                  </div>
                )}
                ...
                <button onClick={inviteGuest} disabled={inviting || !guestName.trim()}
                  ...
                  {inviting ? t('myBookings.processing') : inviteTarget?.payment_mode === 'full' ? t('myBookings.addGuest') : t('myBookings.addGuestWithPrice', { price: inviteTarget?.price_per_player?.toFixed(2) || '' })}
                </button>
              </div>
            )}
          </div>
...
export default function MyBookingsPage() {
  return (
    <Suspense fallback={<MyBookingsLoadingFallback />}>
      <MyBookingsList />
    </Suspense>
  )
}

function MyBookingsLoadingFallback() {
  const { t } = useLocale()
